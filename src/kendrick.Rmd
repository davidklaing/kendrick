...
filtered_track_lyric_titles[117] <- "good kid"
...
filtered_track_lyric_titles[151] <- "i"
...
length(filtered_track_pageviews)
length(filtered_track_annotations)
```
...
track_df$album_name <- factor(track_df$album_name, levels = c("Overly Dedicated", "Section.80", "good kid, m.A.A.d city", "To Pimp A Butterfly", "untitled unmastered.", "DAMN."))

# Add a career-wide track number.
track_df$career_track_number <- 1:dim(track_df)[1]

# Add annotation counts and pageviews
track_df$annotations <- track_annotations
track_df$pageviews <- track_pageviews

head(track_df)
```

```{r viz, fig.width = 12, fig.height=4}
ggplot(track_df, aes(x = track_number, y = annotations, color = album_name)) +
...
ggplot(track_df) +
        geom_point(aes(x = career_track_number, y = acousticness))
```
...
kendrick %>% group_by(album_name) %>% summarise(word_count = sum(song_word_count))
...
# Check out the most common words in the forum.
cleaned_kendrick %>%
...
ggplot(kendrick, aes(x = track_number, y = sentiment, color = sentiment)) +
        ...
        theme_tufte(base_family = 'GillSans') +
        labs(title = "Sentiment in Kendrick Lamar's Lyrics") +
        xlab("") +
        ylab("") +
